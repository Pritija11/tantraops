import type { Metadata } from "next";
import { createMetadata } from "@/lib/seo";

export const metadata: Metadata = createMetadata({
  title: "Security Intelligence | TantraOps",
  description:
    "TantraOps explores security intelligence systems that help organizations understand signals, threats, and changing digital risk.",
  path: "/solutions/security-intelligence",
});

const capabilities = [
  {
    number: "01",
    title: "Security Signals",
    description:
      "Collect and organize relevant information from applications, infrastructure, and systems.",
  },
  {
    number: "02",
    title: "Threat Intelligence",
    description:
      "Turn security-related information into clearer understanding of potential threats.",
  },
  {
    number: "03",
    title: "Detection Support",
    description:
      "Use intelligent systems to identify unusual activity and meaningful security events.",
  },
  {
    number: "04",
    title: "Security Insights",
    description:
      "Transform technical signals into information that can support better decisions.",
  },
];

export default function SecurityIntelligencePage() {
  return (
    <main className="bg-[#0A0D0C] pt-[76px]">
      <section className="border-b border-white/10">
        <div className="mx-auto max-w-7xl px-6 py-24 lg:px-10 lg:py-32">
          <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-[#C7F36B]">
            Solution / Security Intelligence
          </p>

          <h1 className="mt-6 max-w-5xl font-[family-name:var(--font-syne)] text-5xl font-semibold leading-[1.02] tracking-[-0.05em] text-[#F1F4EF] md:text-6xl lg:text-7xl">
            Turn security signals into useful intelligence.
          </h1>

          <p className="mt-8 max-w-2xl text-lg leading-8 text-[#8C9690]">
            Security intelligence brings together technical signals,
            contextual information, and intelligent analysis to help make
            complex environments easier to understand.
          </p>
        </div>
      </section>

      <section className="border-b border-white/10 bg-[#141917]">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-10 lg:py-28">
          <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr]">
            <h2 className="font-[family-name:var(--font-syne)] text-4xl font-semibold tracking-[-0.04em] text-[#F1F4EF]">
              More signals do not always mean more understanding.
            </h2>

            <p className="text-base leading-8 text-[#8C9690]">
              Security environments can generate enormous amounts of technical
              information. Intelligence systems can help organize that
              information, identify important relationships, and surface
              signals that deserve attention.
            </p>
          </div>
        </div>
      </section>

      <section className="border-b border-white/10">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-10 lg:py-28">
          <div className="grid border-l border-t border-white/10 md:grid-cols-2">
            {capabilities.map((item) => (
              <div
                key={item.number}
                className="border-b border-r border-white/10 p-8 lg:p-10"
              >
                <span className="font-mono text-xs text-[#C7F36B]">
                  {item.number}
                </span>

                <h3 className="mt-8 font-[family-name:var(--font-syne)] text-2xl font-semibold text-[#F1F4EF]">
                  {item.title}
                </h3>

                <p className="mt-4 text-sm leading-7 text-[#8C9690]">
                  {item.description}
                </p>
              </div>
            ))}
          </div>
        </div>
      </section>

      <section className="bg-[#141917]">
        <div className="mx-auto max-w-7xl px-6 py-24 lg:px-10 lg:py-32">
          <div className="border border-[#C7F36B]/20 bg-[#0A0D0C] p-8 lg:p-14">
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#C7F36B]">
              Security intelligence
            </p>

            <h2 className="mt-5 max-w-3xl font-[family-name:var(--font-syne)] text-4xl font-semibold tracking-[-0.04em] text-[#F1F4EF] md:text-5xl">
              Understand the signals that matter.
            </h2>

            <p className="mt-6 max-w-2xl text-base leading-8 text-[#8C9690]">
              Explore how intelligent systems can support security visibility,
              analysis, and decision-making.
            </p>

            <a
              href="/contact"
              className="mt-8 inline-flex rounded-full bg-[#C7F36B] px-7 py-3.5 text-sm font-semibold text-[#0A0D0C] hover:bg-[#7ED957]"
            >
              Discuss security intelligence
            </a>
          </div>
        </div>
      </section>
    </main>
  );
}